...
_COMMANDS: dict[tuple[str, ...], tuple[str, list[str], str]] = {
    ...
    # run — one unattended exploration (docs/design-auto.md). Each verb belongs
    # to one phase of the run and is refused in the other.
    ("run", "start"): ("magi.run_cmd", ["start"],
                       "Open a run and its empty contract (phase: discussing)"),
    ("run", "sign"): ("magi.run_cmd", ["sign"],
                      "The person signs: freeze the contract, authorise the run"),
    ("run", "step"): ("magi.run_cmd", ["step"],
                      "Register what is about to be done (refused unless the run may take it)"),
    ("run", "result"): ("magi.run_cmd", ["result"], "Close a step with what came of it"),
    ("run", "amend"): ("magi.run_cmd", ["amend"],
                       "The person takes a signed contract back to discussing"),
    ("run", "outline"): ("magi.run_cmd", ["outline"],
                         "The report's skeleton, with the mechanical parts filled in"),
    ("run", "report"): ("magi.run_cmd", ["report"], "Hand in the run's report and end the run"),
    ("run", "status"): ("magi.run_cmd", ["status"],
                        "The takeover brief: phase, contract, budget, what is in flight"),
    ("run", "stop"): ("magi.run_cmd", ["stop"],
                      "The person ends it: drop a discussion, or have a run write up"),
    ("run", "overturn"): ("magi.run_cmd", ["overturn"],
                          "The person, afterwards: this step should have gone otherwise"),
    # familiar — what the person already knows; the two buttons of design-auto §9
    ("familiar", "list"): ("magi.familiar_cmd", ["list"],
                           "The methods a run used, and what the person said of each"),
    ("familiar", "known"): ("magi.familiar_cmd", ["known"],
                            "The person knows this concept: stop defining it"),
    ("familiar", "notes"): ("magi.familiar_cmd", ["notes"],
                            "The person wants lecture notes on this concept"),
    ("familiar", "forget"): ("magi.familiar_cmd", ["forget"],
                             "Withdraw what was said about a concept"),
    # graph
...
_GROUP_HELP = {
    ...
    "run": "One unattended exploration: a signed contract, then steps any agent can pick up",
    "familiar": "What the person already knows, as they told it (per user, not per project)",
